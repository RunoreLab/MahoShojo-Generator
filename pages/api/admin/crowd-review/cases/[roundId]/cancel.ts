import { cancelAdminCrowdReviewRound } from '@/lib/admin/crowd-review';
import {
...
type HandlerDeps = {
  ...
  getDb: () => any;
  cancelAdminCrowdReviewRound: typeof cancelAdminCrowdReviewRound;
};
...
const getRoundId = (req: Request, context?: HandlerContext): string => {
  ...
  const actionIndex = parts.lastIndexOf('cancel');
...
export const createAdminCrowdReviewRoundCancelHandler =
  ...
  async (req: Request, context?: HandlerContext): Promise<Response> => {
    ...
    try {
      const result = await (
        deps.cancelAdminCrowdReviewRound ?? defaultDeps.cancelAdminCrowdReviewRound
      )({
...
export default withPvpErrorBoundary(createAdminCrowdReviewRoundCancelHandler());
