...
const BattlePage: React.FC = () => {
    ...
    const upsertAdjudicationEvents = useCallback((incoming: AdjudicatorEvent[] | undefined) => {
        ...
        setAdjudicationEvents(prev => {
            const incomingWithKey = incoming.map((evt, idx) => ({ evt, key: buildKey(evt, idx) }));
            const incomingKeys = new Set(incomingWithKey.map(item => item.key));
            const filteredPrev = prev.filter((evt, idx) => !incomingKeys.has(buildKey(evt, idx)));
            return [...filteredPrev, ...incomingWithKey.map(item => item.evt)];
        });
    }, []);
    ...
    const inferCombatantType = (data: any): CombatantData['type'] => {
    ...
    const validateGeneralCharacterData = (data: any, filename: string): { success: boolean } => {
        ...
        if (!parsed.success) {
            ...
            const message = firstIssue?.message ? `：${firstIssue.message}` : '';
    ...
    const handleSelectPreset = async (preset: Preset) => {
        ...
        try {
            ...
            if (preset.type === 'magical-girl') {
                validationSuccess = validateMagicalGirlData(presetData, preset.name).success;
            } else {
                validationSuccess = validateCanshouData(presetData, preset.name).success;
            }
    ...
    const processJsonData = async (jsonText: string, sourceName: string) => {
        ...
        for (const item of dataArray) {
            ...
            try {
                ...
                const type: CombatantData['type'] = template === 'magical-girl'
                    ? 'magical-girl'
                    : template === 'canshou'
                        ? 'canshou'
                        : template === 'general'
    ...
    const handleSelectDataCard = async (cardData: any) => {
        try {
            ...
            if (type === 'magical-girl') {
                const result = validateMagicalGirlData(cleanedCardData, filename);
    ...
    const arenaHistoryReadLimit = readArenaHistory
        ? (isArenaHistoryUnlimited ? null : numericHistoryLimit)
        : undefined;
            ...
            // 加入后置生成敏感词检测
            if (await checkSensitiveWords(JSON.stringify(result.report), {
                source: 'output',
                origin: 'battle',
                reason: '使用危险符文',
    ...
    return (
        <>
            <Head>
                ...
                <meta name="description" content="上传魔法少女、残兽或通用角色的设定，生成她们之间的战斗或日常故事！" />
            </Head>
                        ...
                        <div className="input-group">
                            ...
                            {shouldWarnHistoryLimit && (
                                <p className="text-xs text-orange-600 mt-2">
                                    ⚠️ 当前设置预计将读取{numericHistoryLimit === Infinity ? '无限制数量的' : `约 ${Math.ceil(estimatedHistoryTotal)} 条`}历战记录，超过 20 条可能显著提升生成失败或超时概率。
                                </p>
                            )}
