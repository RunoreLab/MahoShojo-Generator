...
40. 已完成第二批脚本下沉（Auth/维护域）：新增 `lib/db/repositories/user-auth-links-backfill.ts` + `lib/database/user-auth-links-backfill.ts`、`lib/db/repositories/arena-maintenance.ts` + `lib/database/arena-maintenance.ts`，并补充 `lib/db/repositories/pvp-room-core.ts` / `lib/database/pvp.ts` 的清理接口；已迁移脚本包括 `backfill-user-auth-links.ts`、`repair-arena-preset-ids.ts`、`pvp-prune-ephemeral.ts`、`init-large-objects.ts`。  
41. 已完成第三批脚本下沉（season/tags 批次）：新增 `lib/db/repositories/season-archive.ts` + `lib/database/season-archive.ts`、`lib/db/repositories/tags-seed.ts` + `lib/database/tags-seed.ts`，并补齐 `lib/db/schema/business.ts` 的 `tag_aliases` 映射；`scripts/season-archive.ts`、`scripts/init-tags.ts` 已移除脚本内全部 `queryFromD1` 调用。当前脚本域剩余 `queryFromD1` 调用点为 `36`（`season-soft-reset` 与 `badge/*` 批次，`rg "queryFromD1\\(" scripts -n` 本地统计）；本轮改造已通过 `bun run lint`、`bun test`、`bun run build`。  
