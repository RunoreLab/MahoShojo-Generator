...
可选政策变体（你提出的思路）：**仅当对战涉及 Top300 角色时才启用 strict 分差限制**  

- 规则草案：
  - 若双方都不在 Top300：**不做 `strict-out-of-range` 检查**，对战默认可计分（仍保留去重/每日次数/公开审核等其它规则）
  - 若任意一方在 Top300：继续按现有口径做 `strict-out-of-range` 检查
- 好处：在“榜单仅 Top300”且用户主要靠自选对手打严格的前提下，可以显著降低“找不到可计分对手”的挫败感（尤其是低分段玩家）。
- 主要风险（需要明确是否接受）：
  - **可被刷分**：非 Top300 段位内不做分差限制，会放大“喂分/小号/互刷”提升 rating 的空间；一旦刷到 Top300 门槛附近就会影响榜单公信力。
  - **规则断层**：当角色首次进入/掉出 Top300 时，计分规则会突然变化；用户会观察到“昨天能计分，今天同样对手突然不计分”。
  - **实现成本**：结算/预检需要判定“是否在 Top300”，通常要依赖“Top300 列表缓存/快照”。如果每局都去查 Top300，会引入新的 D1 读热点；必须配套强缓存（Edge Cache + 内存）并定义一致的口径与 TTL。
- 更推荐的替代（更稳定且更难被滥用）：
  - **分差限制始终保留**（保持规则一致），但把“找对手”改为“附近分段”接口/列表（上文 1)），该列表不依赖名次、也不要求 Top300 覆盖，因此既能保持严格规则，又能解决可用性问题。
  - 若一定要“低段位放宽”：建议按 **段位/分数阈值** 放宽（例如仅 `白牌/无牌` 放宽或提升 maxAbsDiff），而不是按 Top300 这种硬门槛放宽（门槛型规则更容易产生断层与刷分动力）。
