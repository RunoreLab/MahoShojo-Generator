...
2. **卡组配置（公开）**：支持两种模式：
   - **每人提交**：每人提交 N 张角色卡（来源：预设/公开库/自己的私有库；若 `cardsPerPlayer=0` 则跳过该阶段）。
   - **仅房主提交牌堆**：只有房主提交卡牌（任意张），提交内容作为公共牌堆供所有参与者抽取；其他玩家无需提交卡牌（该模式下不需要设置 `cardsPerPlayer`）。
...
- **去重规则**：按什么去重？`data_card_id` / 预设 `filename` / 内容 hash？去重会让“撞车”成为一种策略（好/坏都要明确）。
  ...
  - 补充（仅房主提交牌堆模式）：由于牌堆由房主统一提交，其他玩家不会产生提交数据；发牌与后续补牌都以房主提交的牌堆为基础，再按房间规则进行去重/洗混/补牌兜底。
...
- 校验：房间“卡牌范围设置”（房主在房间内统一配置）
  - 允许类型：魔法少女 / 残兽 / 通用角色
  - 统计范围：点赞/使用/收藏的最小/最大值（仅对 `data_card` 生效；`preset` 不具备统计字段，仅做类型校验）
  - 注意：若房间允许从“公开库/预设”补牌，则补牌逻辑也应遵守同一套范围，避免绕过限制
- 校验：卡可用性
...
- PVP 结算支持 **non-stream / stream** 两种方式（默认 non-stream）。其中 stream 复用竞技场的流式生成与战报组件：服务端在流结束后落库 `reportMarkdown` 并执行多策略胜者解析；若仍无法确定胜者则自动进入“胜者投票”阶段。
...
### 14.1 结算时出现 Cloudflare `502 Bad gateway` 且显示一大段 HTML
- 多数情况下意味着：边缘函数/上游服务请求超时或异常，Cloudflare 返回了 HTML 错误页。
- **已做体验兜底**：前端会识别并摘要为中文提示，不再把 HTML 整页渲染出来；必要时在控制台保留原始响应片段便于排查。
- 建议排查：记录发生时间、房间号、是否使用自定义供应商（自带 Key）、当时房间阶段与网络情况。

### 14.2 结算“无限转圈”或卡在 `resolving/advancing`
- 可能原因：请求链路挂起/超时、AI 供应商抖动、或某次请求意外中断导致房间停留在中间态。
- 建议操作：刷新页面；房主可尝试“强制重试”；若仍无变化可先退出房间再重新进入。

### 14.3 无法退出房间
- 历史原因：离开接口在推进/结算阶段会拒绝退出，且强依赖 `version` 易触发版本冲突。
- **已修复**：离开接口改为 best-effort，不再强制 `expectedVersion`，busy 阶段也允许退出（即便并发写入失败也会尽力移除成员记录并给出 warning）。
