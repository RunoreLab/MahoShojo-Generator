...
- 2025-12-24：新增“提交卡牌范围设置”：房主可在房间设置中配置允许使用的角色类型（魔法少女/残兽/通用角色）与数据卡统计范围（点赞/使用/收藏的最小/最大值，`null` 表示不限）。该范围会在以下环节生效：①玩家/房主提交卡组时服务端校验；②机器人自动构建卡组时遵守范围；③发牌与补牌时从公开库/预设抽取也遵守范围（不满足类型/统计的卡会被跳过并加入排除集）。若在 `submitting` 阶段修改卡牌范围，将提示清空已提交卡组；同时会重建现有机器人的提交卡组，避免范围不一致。
- 2025-12-24：房间聊天体验改进：修复“发送表情包/emoji 必然附带句式文字”的设计问题，允许用户单独发送任意一类内容；新增固定快捷消息表（`config/pvp-chat-quick-messages.json`，按“同意/反对/致歉/催促”分组展示，并支持“最近常用”）；聊天与计分板的用户展示与玩家列表对齐，可正确显示已佩戴徽章；表情包资源按静态 `public/` 路径加载，支持 `svg/png/jpg/jpeg/webp/gif` 等常见格式（由 `config/pvp-chat-emotes.json` 的 `src` 决定）。
- 2025-12-24：PVP 结算新增“流式生成（实验性）”：房间规则新增 `generationMode=non-stream|stream`；新增接口 `POST /api/pvp/rooms/:roomId/rounds/:roundId/resolve-stream`，流式返回 Markdown 战报并在流结束后落库 `latestRoundResult.reportMarkdown`；胜者解析采用多策略（meta → “## 胜利者”首行 → 正则兜底），失败则自动进入“胜者投票”。
...
- `lib/pvp/validate.ts:1`（规则校验：允许 `cardsPerPlayer=0` 跳过提交；不再强制 `cardsPerPlayer > dealPerPlayer`）
