...
- **批量操作**：对所选角色执行“生成更新/应用更新/下载更新后角色卡/保存到云端/替换已有（云端）”，降低多角色管理成本。
...
- **依赖型角色卡**  
  - 小城市的魔法少女 1.7 要求携带 templateId=fairy 妖精卡（如 [妖精]兰兰）。缺失需输出硬错误提示。
- **叙事禁词 vs 记录允许冲突**  
  - 黑烬黎明地下炼狱系列：正文禁词，但记录必须使用“体力值/饥饿度”等术语。
...
**A. 叙事生成阶段（正文）**
- 明确要求 AI：**忽略**角色/情景卡中关于“写入摘要/状态/历战/选项”的要求。
- 仅允许读取 current_state/arena_history（若存在）作为剧情连续性参考。
- 若卡内有“缺少依赖卡需报错”要求：改为输出可解析提示信号（见 19.4），而不是写入 officialReport.*。

**B. 记录更新阶段（current_state/arena_history）**
- 明确要求 AI：**必须遵守**角色/情景卡内关于写入格式的要求。
- 该阶段允许使用“记录术语”，并**忽略叙事禁词**限制。
- 若同时存在多套协议（不同角色/情景卡），允许分角色分别遵守。

**C. 选项生成阶段（配合全局数据卡）**
- 明确要求 AI：**必须遵守**角色/情景卡内关于选项生成与格式的要求。
- 输出统一结构化格式（见 19.4），再由程序写入“全局数据卡”。

**D. 摘要生成阶段（可选）**
- 若卡内存在“摘要/结算格式”要求，则按要求输出；否则沿用茶会通用摘要模板。

### 19.4 可解析输出规范（替代 officialReport.*）

为兼容卡内“硬错误/提示/选项”要求，统一使用可解析信号块（示意）：

```mtp_notice
{"type":"error","code":"missing_required_card","message":"未检测到播种者妖精，请检查是否放入“[妖精]兰兰”。"}
```
...
> 前端解析 mtp_notice / mtp_choices 后再写入 UI 或工具型数据卡。

### 19.5 自动补齐与读取规则（与竞技场对齐）

- **写入时自动补齐**：当写入 current_state/arena_history 时若字段不存在，自动补齐结构：  
  - current_state: { summary: "", updated_at: now }  
  - arena_history: []  
  - 适用于所有角色/工具卡，不仅限“工具型数据卡”。
- **读取时缺省**：若角色/工具卡没有对应字段，视为“无记录可读”，不额外报错。
- **工具卡识别**：以 codename / templateId 命中（如 “全局数据卡”）后参与读写。
...
- **风险：提示信号被正文污染**  
  - 处理：强制 mtp_notice / mtp_choices 独立块输出，解析失败即丢弃并提示。

### 19.7 落地清单（方向对齐）

- 为叙事/记录/选项三种生成路径分别新增“高优先级系统提示词”模板。  
- 增加 mtp_notice / mtp_choices 解析与 UI 显示逻辑。  
