...
- 将“用户输入/选项选择”视作 `userGuidance`：若用户选择选项，提示词需附带“选项编号 + 原文”。
- 仅允许读取 current_state/arena_history（若存在）作为剧情连续性参考（此项可以不修改提示词要求，已经在现有的读/写机制中实现）。
- 若卡内有“缺少依赖卡需报错”要求：改为输出可解析提示信号（见 19.4），并**终止本轮正文输出与写入流程**。
...
- 选项原文需回传给记录更新阶段，用于写入“全局数据卡”的 current_state.summary / arena_history.impact（若协议要求）。
- 若卡内强制要求选项但用户未触发选项生成，输出 `notice(level=warning)` 提示用户补充生成选项。
