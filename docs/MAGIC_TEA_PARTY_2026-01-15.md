...
- **卡片内联展示**：
  ...
  - 状态字段（`current_state.fields`）可折叠显示，允许手动增删改（提示“修改会影响原生性/签名”）。
- **历战记录模态框**：
  - 列表展示条数较多时，提供搜索/筛选（type/时间/参与者）与分页。
  - 默认只允许**删除**或**标记无效**；如需编辑或新增，提供“高级编辑模式”并提示签名/原生性风险。
...
**方案 E：混合（推荐）**
- 在“摘要生成/关键里程碑”后**自动生成更新草案**，由用户确认是否写入。
...
- **触发点**：以 `SummaryPanel` 的“会话摘要生成”作为主要写入触发；当无摘要时提示用户先生成摘要。
- **复刻预设**：当启用竞技场预设（classic/kizuna/daily），默认继承竞技场读写配置，但仍遵循“用户确认写入”。
...
- **字段编辑**：交由角色管理组件手动修改，配合“原生性提示/重新签名”逻辑。
...
- **类型映射**：
  - 方案 A：沿用 `type='scenario'`（零改动、兼容性最好）。
  - 方案 B：新增 `type='tea-party'`（语义更清晰，但需扩展类型与下游逻辑）。
- **胜者字段**：若无明确胜负，填充“无胜者/未判定”，并在 `metadata.source='magic-tea-party'` 中标记，便于下游过滤。
...
- **原生性**：由服务端签名验证并重新签名；若检测冲突则降级为非原生（与竞技场一致）。
