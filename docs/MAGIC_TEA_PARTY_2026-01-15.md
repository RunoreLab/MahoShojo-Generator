...
- `article.analysis` / `正确游玩指引`：映射为 `notice(level=info)` 或首轮/卡组变化提示。
...
- 若卡片**强制要求选项**但用户关闭了选项功能：允许**单轮强制生成**（不持久修改用户设置），并提示“协议要求选项已临时开启”；若用户仍拒绝生成，则输出 `notice(level=warning)` 并跳过与选项绑定的写入。
