...
- 对已知依赖型卡维护白名单依赖校验，提前阻断缺卡场景（见 19.9）。

### 19.9 依赖卡校验策略（定稿：内置卡白名单）

**选择理由（权衡）**
- **内置卡白名单**：实现成本低、风险低，不需要改动角色管理与竞技场数据流，适合设计阶段快速落地与迭代。  
- **外部卡 meta 标记**：扩展性强但牵涉 UI/数据卡管理/竞技场对齐与迁移，当前阶段成本过高。

**策略描述**
- **范围**：仅对内置或官方维护的“已知依赖型卡”启用依赖校验。  
- **触发**：叙事阶段启动前先校验依赖，缺失则输出 `notice(level=error)` 并终止本轮正文/写入流程。  
- **例子**：`小城市的魔法少女 1.7` 缺少 `templateId=fairy` 角色卡时提示用户加入 `[妖精]兰兰`。  
- **落地形式**：在茶会侧维护一份轻量映射表（`scenarioId -> requiredRolePredicate`），仅用于提示与拦截，不参与卡片解析。

**后续扩展（可选）**
- 若未来需要支持外部卡依赖：建议引入 `meta.required_roles`（或 `dependencies`）字段，并同步角色管理与竞技场的“缺卡提示 + 一键补齐”体验。
