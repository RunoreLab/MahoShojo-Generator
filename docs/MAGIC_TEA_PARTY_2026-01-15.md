...
- 若卡内有“缺少依赖卡需报错”要求：改为输出可解析提示信号（见 19.4），而不是写入 officialReport.* 或其他魔法茶会不存在的字段。
...
- **风险：提示信号被正文污染**  
  - 处理：强制 mtp_notice 独立块输出，解析失败即丢弃并提示。

### 19.7 落地清单（方向对齐）

- 为叙事/记录/选项三种生成路径分别新增“高优先级系统提示词”模板。  
- 增加 mtp_notice 解析与 UI 显示逻辑。  
