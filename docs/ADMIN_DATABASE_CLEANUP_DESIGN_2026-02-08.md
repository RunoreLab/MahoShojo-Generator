...
## 0. 当前实现进度（2026-02-09 更新）

> 本节用于记录当前 `feat/admin` 分支的落地状态，后续开发以此为准。

- 已落地统一页面：`/admin/data-maintenance`，支持目标选择、范围配置、动作编排、预览、执行；
- 已落地后端能力：`preview/execute/jobs/jobs/[id]` 四组 API；
- 已落地字段级操作：`truncate`、`set_null_or_default`、`delete_rows`（含 `large_objects` 可选联动删 R2）；
- 已落地安全护栏：`planHash` 校验、`delete_rows` 口令确认、白名单目标/字段/动作；
- 已落地任务审计：`admin_cleanup_jobs`、`admin_cleanup_job_logs`；
- 已补充可运维能力：执行历史状态筛选、任务详情查看、从历史任务一键回填表单配置；
- 已增强复用体验：支持“复用并立即预览”与“当前表单 vs 历史任务”差异高亮。

### 0.1 当前约束（与原设计差异）

- 鉴权策略暂缓（用户已确认）：由于线上分支不含后台页面，`feat/admin` 仅管理员本机使用，现阶段先不阻断开发；
- 后续上线前必须补齐第 9 节鉴权要求，并完成一次全量安全复核。
...
- Cloudflare D1 在 `IN (?, ?, ...)` 这类语句上存在绑定变量数量上限；当单批参数过多时会返回  
  `too many SQL variables`（常见于 `batchSize` 设得较大、且动作为 `delete_rows` 或字段批量更新）。
- 现已在执行引擎加入**自动降级分片**：若命中该错误，会把当前 ID 批次递归拆分为更小子批次后重试。
- 因此页面层 `batchSize` 主要用于控制任务吞吐与日志粒度，不再需要严格手动对齐 SQL 变量上限。
- 建议仍从中等值开始（如 `200~500`），并根据耗时与数据库负载逐步调整。
