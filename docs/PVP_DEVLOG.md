...
- 2025-12-24：房间聊天体验改进：修复“发送表情包/emoji 必然附带句式文字”的设计问题，允许用户单独发送任意一类内容；新增固定快捷消息表（`config/pvp-chat-quick-messages.json`，含“同意/不同意/对不起”等）；聊天与计分板的用户展示与玩家列表对齐，可正确显示已佩戴徽章；表情包资源按静态 `public/` 路径加载，支持 `svg/png/jpg/jpeg/webp/gif` 等常见格式（由 `config/pvp-chat-emotes.json` 的 `src` 决定）。
