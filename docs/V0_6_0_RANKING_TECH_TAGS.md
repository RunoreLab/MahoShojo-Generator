...
- 无牌：`games < placementGames`（例如 5）或 `rating < 800`
- 白牌：`800–999`
- 字牌：`1000–1199`
- 花牌：`1200–1499`
- 权杖：`>= 1500`
...
细分 I/II/III 的两种方式：
- 方式 A：每档 100 分拆 3 段（例如 800-899/900-949/950-999）
...
- **历史归档**：赛季结束时，生成的历史快照存储于 `public/data/seasons/archive_{season_id}.json`。客户端读取该静态文件，并复用当前赛季的排序/筛选规则在前端生成各口径榜单（见 8.1.1）。
- **作者与统计快照**：归档时会写入榜单所需的关键统计字段（不保存数据卡 `data` 正文，避免体积过大）。包括作者 `authorName/authorId`、点赞/收藏/使用量、创建/更新时间、技术值/原生性、标签、段位/排位分/对局数/W-L-D 等。

**8.1.1 归档内容取舍：只存实体快照 vs 存 leaderboards（2026-02-05 补充）**

先把 `archive_<season_id>.json` 里的数据分成两层：
- **实体快照（facts）**：每个角色在赛季结束时的可验证数据（strict/free 的 `rating`、`games`、`W/L/D`、更新时间、技术值/标签、作者等）。
- **榜单视图（views）**：由 facts 按某个口径（排序/筛选/是否包含预设/标签规则等）计算出来的排名列表与名次。
...
- **更易维护**：归档 schema 更简单，迁移成本更低，也更利于长期演进。

但需要提前说明两点（否则容易踩坑）：
- **冻结 vs 重算**：如果历史榜单要求“永远与结算时一致”，那么仅存 facts 并用“当前代码”重算会存在漂移风险（tie-breaker、段位阈值、是否包含预设/标签规则等任何变化都会改写历史显示）。这类诉求下应当：
  - 要么在归档里记录 `rulesVersion` / `rankingRulesVersion` 并保证旧规则可回放；
  - 要么保存最少的“权威顺序”（例如每队列一份 `orderedEntityKeys`，或每实体一个 `rankByRating`），把它视为“结算时的事实之一”。
- **快照范围与性能**：
  - 若只归档 Top/Bottom 子集，历史页无法复现“全榜”分页与筛选；此时仍应保留 `totalEligible`/`snapshotPolicy` 等元信息，避免用户误解。
  - 若改为归档全量实体，文件体积会增大；可通过 gzip/brotli、按队列分文件、或 Worker/懒加载来减轻前端排序成本。
...
1. **冻结与归档**：
   - 运行归档脚本，锁定当前榜单。
   - 提取全服 Top 100（最强）与 Bottom 50（最弱）的角色排行榜所需信息快照，写入静态文件（当前实现）。若希望历史赛季支持“与当前赛季一致的完整排序/筛选/分页”，应改为归档全量实体快照。
