...
import { POST as repairCombatantMeta } from '../src/adapters/arena/repair-combatant-meta';
...
describe('Hono Arena companion adapters', () => {
  it('四条路由委托给同一 shared service 并携带准确 operation', async () => {
    const service = {
      ...
      repairCombatantMeta: vi.fn(async () => new Response('repair')),
    };
    ...
    const responses = await Promise.all([
      ...
      repairCombatantMeta(new Request('https://example.test/api/arena/repair-combatant-meta', { method: 'POST' })),
    ...
    expect(service.repairCombatantMeta).toHaveBeenCalledTimes(1);
