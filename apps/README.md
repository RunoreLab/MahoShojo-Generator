...
Phase 2.5C 已激活 Hono API 的 source、manifest、测试和构建边界；G3-P0 已建立 Admin 的独立 deny-all
pre-work shell 与安全原语，其他 runtime 仍按阶段计划渐进迁移，不能据此宣称正式 Phase 3 或生产 Admin 已解锁。

长期目标边界如下：

- `web`：已激活的 Next/OpenNext workspace app，独占 Web source、测试、静态资产、环境与 Cloudflare deployment lifecycle；
- `admin`：G3-P0 已激活的独立 Worker/BFF pre-work 边界，当前只有安全壳和 Access/RBAC/CSRF/audit 原语，
  无业务数据能力、生产 route 或自动部署，详见 [`admin/README.md`](./admin/README.md)；
- `api`：已激活的 Hono Node workspace app，持有 `23 shared-service / 6 exited / 0 legacy-next`
...
除 `apps/web`、`apps/api`、`apps/d1-gateway` 与上述 G3-P0 `apps/admin` 边界外，README 或空目录仍只表示
路线图占位。当前没有迁移 Admin 业务域、Desktop、Mobile 或完整 Arena Room runtime。应用之间不得直接导入彼此内部源码，共享能力应经
