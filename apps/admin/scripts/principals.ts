...
if (!['bootstrap', 'revoke', 'restore', 'status'].includes(command)) throw new Error('Usage: node scripts/principals.mjs status|bootstrap|revoke|restore --local|--remote [--confirm-remote]');
...
try {
  ...
  if (args.includes('--remote')) {
    if (command !== 'status' && !args.includes('--confirm-remote')) throw new Error('Remote principal changes require --confirm-remote');
