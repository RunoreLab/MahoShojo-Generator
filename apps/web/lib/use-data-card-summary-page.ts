...
export function useDataCardSummaryPage(source: 'my' | 'favorites', ownerId: number | null, enabled: boolean, query: DataCardSummaryQueryInput) {
  ...
  useEffect(() => { setCards([]); setTotal(0); setHasLoaded(false); setStats(undefined); setStatus('idle'); setError(null); }, [ownerId, source]);
  useEffect(() => {
  ...
  }, [source, ownerId, enabled, queryKey, refresh]);
  useEffect(() => {
