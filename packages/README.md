...
- `@mahoshojo/hosted-api`：承载 Hono 与 Next/OpenNext 共同调用的 Hosted application service；首个真实纵切为 `generate-magical-girl`，统一请求校验、限速/安全/生成/活动记录/签名顺序和 wire response，并通过端口注入 AI、限速、安全、签名与日志实现。该包只依赖 Web Standards 与 Zod，不导入 Next、Hono、环境变量、服务器秘密或 legacy root app；当前一条 shared service 已有 Next 与 Hono 两个 adapter，剩余 Hono 白名单仍按 Phase 2.5B 渐进收敛；
