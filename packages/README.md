...
- `@mahoshojo/ai-direct`：当前只导出 runtime-neutral `AiExecutionPort`，统一 `execute` 与可取消 `stream` 的 adapter 边界；Direct Provider Profile、Secure Vault、Native/Main executor、Endpoint transport policy 与具体 Provider adapter 尚未实现；
