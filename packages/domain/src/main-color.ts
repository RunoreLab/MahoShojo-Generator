...
export const MAIN_COLOR_VALUES = Object.values(MainColor) as [MainColorLabel, ...MainColorLabel[]];

export const COLOR_GRADIENTS: Record<
  MainColorLabel,
...
export const getMainColorGradient = (
  key: MainColorKey | null | undefined,
): { first: string; second: string } => {
  const resolvedKey: MainColorKey = key && key in MainColor ? key : 'Pink';
