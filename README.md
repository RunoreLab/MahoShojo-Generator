...
v0.7.1 带来新的重量级功能：**魔法茶会**——一个基于角色卡/情景卡的“长期剧情对话”页面，适合写长线故事与慢慢养成角色关系。

- **魔法茶会（长期对话）**：`/magic-tea-party`（本地会话/多角色同台/主情景+追加情景）
- **JSONL / Markdown 输出**：JSONL 可解析旁白/对白/选项；Markdown 更自由但不保证稳定解析
- **视觉小说选项**：一键生成 2~16 条下一步行动选项
- **自动摘要**：对话变长时可自动摘要（可在全局设置关闭）
- **角色更新**：可把影响写入 `arena_history` / `current_state`（写入会移除 `signature`，变为非原生）
- **协议适配**：支持 `mtp_notice` 可解析提示/报错（用于强协议卡的依赖/缺输入提示）
- **教程入口**：`/encyclopedia`（魔法茶会分类已新增条目）

## 🆕 v0.7.0（创作者工具箱）

与上一版（排位/技术值/标签）偏“强度党”不同，v0.7.0 更偏向 **创作者**：提供更自由的写设定/做舞台/跨生态迁移的工具链。
...
- **酒馆生态联动**：`/tavern` 支持 SillyTavern 角色卡 PNG 导入/导出
- **角色组队卡**：`/character-party` 把多张角色卡拼成一张“队伍卡”
- **教程入口**：`/encyclopedia`（通用卡/自由生成/酒馆联动/组队卡等）
