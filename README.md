...
- **实时流式生成**：实时观看战报生成过程
- **多人模式**：打开多人房间邀请朋友一起配置与围观战报；成员可将修改作为提案发给房主审阅，也可每人带一个角色写行动引导，像多人跑团一样由房主汇总生成
- **断线恢复与容灾**：长生成过程中网络短暂中断后可重新接上原任务；主服务异常时，符合条件的请求可自动改用备用服务路径
...
* **主服务 API**: Hono（Node.js）+ Redis（Arena 多人房间与可恢复生成运行时）
* **语言**: TypeScript
* **包管理器**: pnpm 11.3.0
* **运行时**: Node.js 22+ (开发、构建与脚本), Cloudflare Pages/Workers (Web 前端与容灾备用路径，Edge Runtime)
* **数据库**: Cloudflare D1（主库）+ Cloudflare R2（大对象存储）
