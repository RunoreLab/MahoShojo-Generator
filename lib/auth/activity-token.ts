...
export const issueActivityToken = async (
  userId: number,
...
export const verifyActivityToken = async (
  token: string,
  options?: { now?: Date },
  ...
  const ok = await verifySignature(parsed);
  if (!ok) return null;
...
export const getUserIdFromActivityHeaders = async (headers: Headers): Promise<number | null> => {
  const token = headers.get(ACTIVITY_TOKEN_HEADER);
  if (token) {
    const verified = await verifyActivityToken(token);
    if (verified) return verified.userId;
  }

  return parseActivityUserIdHeader(headers.get(ACTIVITY_USER_ID_HEADER));
};
