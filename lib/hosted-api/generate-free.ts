...
export const createDefaultGenerateFreeService = (): GenerateFreeService =>
  createGenerateFreeService<FreeGeneration, FreeGeneration>({
    ...
    generate: async (_request, input) => {
      ...
      const schemaId = input.schema as FreeSchemaId;
      const schema = schemaMap[schemaId];
      const fieldGuide = buildFieldGuide(schemaId);
      const generationConfig: GenerationConfig<
        any,
        { prompt: string; language: string; attachments: AITextAttachment[] }
      > = {
        systemPrompt: '你的任务是创作具有指定数据结构的内容。',
        temperature: 0.7,
        promptBuilder: (generationInput) => `
...
内容语言：请使用【${generationInput.language}】撰写所有自然语言字段。

${fieldGuide}

${formatReferenceAttachmentsForPrompt(generationInput.attachments)}
...
`.trim(),
      ...
      const aiOptions = providerOptions
        ? { ...providerOptions, channelContext, telemetry }
        : { channelContext, telemetry };
    ...
    normalizeOutput: async (_request, input, generated) => {
      const schemaId = input.schema as FreeSchemaId;
      const sanitized = sanitizeFreeCard(schemaId, generated.result);
      return completeStep({
        result: validateForApp(schemaId, sanitized),
