// Hosted generate-scenario default runtime composition.
...
export const createDefaultGenerateScenarioService = (): GenerateScenarioService =>
  createGenerateScenarioService<ScenarioGeneration>({
    ...
    generate: async (_request, input: GenerateScenarioInput) => {
      ...
      const generationConfig = createGenerationConfig(
        ...
        input.language,
        input.fieldsToKeepEmpty as string[],
      );
      if (customModelOverride) generationConfig.modelOverride = customModelOverride;
    ...
    logError: (error) => log.error('情景生成失败', { error }),
  });
