...
const assertPreviewD1Provisioned = (options) => {
  ...
  const environmentPath = resolve(process.cwd(), 'config/preview-environment.json');
  const wrangler = parse(readFileSync(wranglerPath, 'utf8'), undefined, true);
  ...
  const productionDatabases = [
    ...(Array.isArray(wrangler?.d1_databases) ? wrangler.d1_databases : []),
  ...
  const productionIds = new Set(
    productionDatabases
      ...
      .filter((id) => typeof id === 'string' && D1_UUID_PATTERN.test(id)),
  );
