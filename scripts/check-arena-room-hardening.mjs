...
const EXPECTED_DRILLS = Object.freeze([
  ...
  },
  {
    ...
    assertions: [
      ...
      '新 actor 使用新 epoch warm recovery，旧 epoch 已写入 incarnation fence set',
      '停机窗口 checkpoint raw 保持一致，恢复后 roomId 与 member identity 可读',
    ],
  },
  {
  ...
  },
  {
    ...
    assertions: [
      '已连接真实 socket 收到 1012；gateway.prepareUpgrade 在 shutdown 后返回 503',
